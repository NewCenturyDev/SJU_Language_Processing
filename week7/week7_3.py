...
from SentenceSimilarityModel import SentenceSimilarityModel
...
def do_train(model, q1_data, q2_data, labels):
    ...
    checkpoint_path = DATA_OUT_PATH + 'cnn_similarity' + '/weights.h5'
...
def do_test(model, test_q1_data, test_q2_data, test_id_data):
    ...
    model.load_weights(os.path.join(DATA_OUT_PATH, 'cnn_similarity', save_file_name))
...
cnn_model = define_model_hyperparams(train_data['configs'])
do_train(cnn_model, train_data['q1'], train_data['q2'], train_data['labels'])
test_data = load_test_data()
do_test(cnn_model, test_data['q1'], test_data['q2'], test_data['id'])
