...
def save_result_to_csv(test_id, test_predicted):
    ...
    answer_dataset = pd.DataFrame(data={'id': list(test_id), 'sentiment': list(test_predicted)})
    answer_dataset.to_csv(DATA_OUT_PATH + 'movie_review_result_rnn.csv', index=False, quoting=3)
